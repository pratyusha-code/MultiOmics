...
KIRC_sparse<- read.csv("latent representations from autoencoder/latentRepresentation_KIRC_Sparse.csv")

#converting into dataframe 

KIRC_sparse<- data.frame(KIRC_sparse)
print(dim(KIRC_sparse))


#replacing all the "TCGA.A2.A04U.01A.11R.A115.07" pattern to "TCGA-A2-A04U-01A-11R-A115-07" for pacients col
KIRC_sparse$Pacients <- gsub(".", "-", KIRC_sparse$Pacients, fixed = TRUE)

# Extract the first 3 parts of each TCGA barcode
KIRC_sparse$Pacients <- substr(KIRC_sparse$Pacients, start = 1, stop = 12)
...
for (clusterNum in 3:6) {
  ...
  p_value <- survAnalysis(mainTitle = paste0("KM_Eucli_KRCC_F1000_100F_sparse_"), merge_data$time, merge_data$status, group, distanceMatrix, similarity = TRUE)
...
for (clusterNum in 3:6) {
  ...
  # Perform survival analysis
  p_value <- survAnalysis(mainTitle = paste0("PAM_spearman_KRCC_F1000_100F_sparse"), merge_data$time, merge_data$status, group, distanceMatrix, similarity = TRUE)
