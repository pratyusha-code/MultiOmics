...
latent_rep_data <- read.csv("latent representations from autoencoder/latentRepresentation_COAD_Sparse.csv")
...
#survival analysis
p_value=survAnalysis(mainTitle="KM_Eucli_COAD_F1000_100F_Sparse_",merge_data$time,merge_data$status,group,
...
#survival analysis
p_value1=survAnalysis(mainTitle="PAM_spearman_COAD_F1000_100F_Sparse",merge_data$time,merge_data$status,group,
                      distanceMatrix,similarity=TRUE)
...
for (clusterNum in 3:6) {
  ...
  p_value <- survAnalysis(mainTitle = paste0("KM_Eucli_COAD_F1000_100F_Sparse_"), merge_data$time, merge_data$status, group, distanceMatrix, similarity = TRUE)
...
for (clusterNum in 3:6) {
  ...
  # Perform survival analysis
  p_value <- survAnalysis(mainTitle = paste0("PAM_spearman_COAD_F1000_100F_Sparse_"), merge_data$time, merge_data$status, group, distanceMatrix, similarity = TRUE)
