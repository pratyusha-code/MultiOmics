...
latent_rep_data <- read.csv("latent representations from autoencoder/latentRepresentation_BIC_Denoising.csv")
...
#survival analysis
p_value=survAnalysis(mainTitle="KM_Eucli_BRCA_F1000_100F_Denoising_",merge_data$time,merge_data$status,group,
...
#survival analysis
p_value1=survAnalysis(mainTitle="PAM_spearman_BRCA_F1000_100F_Denoising",merge_data$time,merge_data$status,group,
                      distanceMatrix,similarity=TRUE)
...
for (clusterNum in 3:6) {
  ...
  result <- ExecuteCC(clusterNum = clusterNum, seperate_sub_data_features, maxK = 10, clusterAlg = "km", distance = "euclidean", title = "BRCA")
  ...
  p_value <- survAnalysis(mainTitle = paste0("KM_Eucli_BRCA_F1000_100F_Denoising_"), merge_data$time, merge_data$status, group, distanceMatrix, similarity = TRUE)
...
for (clusterNum in 3:6) {
  ...
  # Perform survival analysis
  p_value <- survAnalysis(mainTitle = paste0("PAM_spearman_BRCA_F1000_100F_Denoising"), merge_data$time, merge_data$status, group, distanceMatrix, similarity = TRUE)
