...
latent_rep_data <- read.csv("latent representations from autoencoder/latentRepresentation_BIC_VARIATIONAL.csv")
...
BRCA_clinical_data <- readRDS("D:/paper implementation - I/mutliview_subtype_data_code/original_data/BRCA_clinical_data.rds")
BRCA_clinical_data <- as.data.frame(BRCA_clinical_data %>% distinct())

#creating subset of dataframes for the relevant columns
#df_clinical <- subset(df, select=c("case_submitter_id","vital_status","days_to_death","days_to_last_follow_up","ajcc_pathologic_stage"))


###merging "Pacients" and "case_submitter_id" 
lr <- sub_data
cd <- BRCA_clinical_data
cat(dim(lr), dim(cd))
...
result=ExecuteCC(clusterNum=3,seperate_sub_data_features,maxK=10,clusterAlg="km",distance="euclidean",title="BRCA")
result
...
#survival analysis
p_value=survAnalysis(mainTitle="KM_Eucli_BIC_F1000_100F_variational_",merge_data$time,merge_data$status,group,
                     distanceMatrix,similarity=TRUE)


#without loop and using PAM and spearman distance

result1=ExecuteCC(clusterNum=3,seperate_sub_data_features,maxK=10,clusterAlg="pam",distance="spearman",title="BRCA")
result1
...
#survival analysis
p_value1=survAnalysis(mainTitle="PAM_spearman_BIC_F1000_100F_VARIATIONAL",merge_data$time,merge_data$status,group,
                      distanceMatrix,similarity=TRUE)
...
for (clusterNum in 3:6) {
  ...
  result <- ExecuteCC(clusterNum = clusterNum, seperate_sub_data_features, maxK = 10, clusterAlg = "km", distance = "euclidean", title = "BIC")
  ...
  p_value <- survAnalysis(mainTitle = paste0("KM_Eucli_BRCA_F1000_100F_VARIATIONAL"), merge_data$time, merge_data$status, group, distanceMatrix, similarity = TRUE)
...
for (clusterNum in 3:6) {
  # Execute clustering analysis
  result1 <- ExecuteCC(clusterNum = clusterNum, seperate_sub_data_features, maxK = 10, clusterAlg = "pam", distance = "spearman", title = "BIC")
  ...
  # Perform survival analysis
  p_value <- survAnalysis(mainTitle = paste0("PAM_spearman_BRCA_F1000_100F_VARIATIONAL"), merge_data$time, merge_data$status, group, distanceMatrix, similarity = TRUE)
