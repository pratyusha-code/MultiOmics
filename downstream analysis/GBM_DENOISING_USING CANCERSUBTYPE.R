...
latent_rep_data <- read.csv("latent representations from autoencoder/latentRepresentation_GBM_Denoising.csv")
...
for (clusterNum in 3:6) {
  ...
  p_value <- survAnalysis(mainTitle = paste0("GBM_KM_Eucli_F1000_100F_Denoising"), merge_data$time, merge_data$status, group, distanceMatrix, similarity = TRUE)
...
for (clusterNum in 3:6) {
  ...
  # Perform survival analysis
  p_value <- survAnalysis(mainTitle = paste0("GBM_PAM_SPRMN_F1000_100F_Denoising"), merge_data$time, merge_data$status, group, distanceMatrix, similarity = TRUE)
