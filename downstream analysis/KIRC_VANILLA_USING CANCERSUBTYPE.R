...
KIRC_vanilla<- read.csv("latent representations from autoencoder/latentRepresentation_KIRC_Vanilla.csv")

#converting into dataframe 

KIRC_vanilla<- data.frame(KIRC_vanilla)
print(dim(KIRC_vanilla))


#replacing all the "TCGA.A2.A04U.01A.11R.A115.07" pattern to "TCGA-A2-A04U-01A-11R-A115-07" for pacients col
KIRC_vanilla$Pacients <- gsub(".", "-", KIRC_vanilla$Pacients, fixed = TRUE)

# Extract the first 3 parts of each TCGA barcode
KIRC_vanilla$Pacients <- substr(KIRC_vanilla$Pacients, start = 1, stop = 12)
...
print(KIRC_clinical_cases$case_submitter_id)
...
merge_data<- merge_data %>% select(-c(days_to_death,days_to_last_follow_up,deceased))
...
#survival analysis
p_value=survAnalysis(mainTitle="KM_Eucli_KRCC_F1000_100F_Vanilla_",merge_data$time,merge_data$status,group,
...
#survival analysis
p_value1=survAnalysis(mainTitle="PAM_spearman_KRCC_F1000_100F_Vanilla",merge_data$time,merge_data$status,group,
                      distanceMatrix,similarity=TRUE)
...
for (clusterNum in 3:6) {
  ...
  p_value <- survAnalysis(mainTitle = paste0("KM_Eucli_KRCC_F1000_100F_Vanilla_"), merge_data$time, merge_data$status, group, distanceMatrix, similarity = TRUE)
...
for (clusterNum in 3:6) {
  ...
  # Perform survival analysis
  p_value <- survAnalysis(mainTitle = paste0("PAM_spearman_KRCC_F1000_100F_Vanilla_"), merge_data$time, merge_data$status, group, distanceMatrix, similarity = TRUE)
