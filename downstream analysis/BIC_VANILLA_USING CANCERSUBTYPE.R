...
latent_rep_data <- read.csv("latent representations from autoencoder/latentRepresentation_BIC_Vanilla.csv")
...
#survival analysis
p_value=survAnalysis(mainTitle="BRCA_3",merge_data$time,merge_data$status,group,
...
#survival analysis
p_value1=survAnalysis(mainTitle="BRCA_3",merge_data$time,merge_data$status,group,
                     distanceMatrix,similarity=TRUE)
...
for (clusterNum in 3:6) {
  ...
  p_value <- survAnalysis(mainTitle = paste0("BRCA_", clusterNum), merge_data$time, merge_data$status, group, distanceMatrix, similarity = TRUE)
