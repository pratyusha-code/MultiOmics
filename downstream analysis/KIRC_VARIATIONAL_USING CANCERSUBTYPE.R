...
latent_rep_data <- read.csv("latent representations from autoencoder/latentRepresentation_KRCC_VARIATIONAL.csv")
print(dim(latent_rep_data))

#converting into dataframe 

latent_rep_data<- data.frame(latent_rep_data)
print(dim(latent_rep_data))


#replacing all the "TCGA.A2.A04U.01A.11R.A115.07" pattern to "TCGA-A2-A04U-01A-11R-A115-07" for pacients col
latent_rep_data$Pacients <- gsub(".", "-", latent_rep_data$Pacients, fixed = TRUE)

# Extract the first 3 parts of each TCGA barcode
latent_rep_data$Pacients <- substr(latent_rep_data$Pacients, start = 1, stop = 12)
...
#survival analysis
p_value=survAnalysis(mainTitle="KM_Eucli_KRCC_F1000_100F_sparse_",merge_data$time,merge_data$status,group,
...
#survival analysis
p_value1=survAnalysis(mainTitle="PAM_spearman_KRCC_F1000_100F_sparse",merge_data$time,merge_data$status,group,
                      distanceMatrix,similarity=TRUE)
...
for (clusterNum in 3:6) {
  ...
  p_value <- survAnalysis(mainTitle = paste0("KM_Eucli_KRCC_F1000_100F_VARIATIONAL_"), merge_data$time, merge_data$status, group, distanceMatrix, similarity = TRUE)
...
for (clusterNum in 3:6) {
  ...
  # Perform survival analysis
  p_value <- survAnalysis(mainTitle = paste0("PAM_spearman_KRCC_F1000_100F_VARIATIONAL"), merge_data$time, merge_data$status, group, distanceMatrix, similarity = TRUE)
